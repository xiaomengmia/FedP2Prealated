...
class SAFA_MQTT:
  ...
  def Safa_MQTT_Train(self, network, train_loader, optimizer , epoch, dev, log_interval, train_losses, train_counter, param, hostname, criterion):
    network.train()
  ...
  def Safa__MQTT_Test(self, network, param, test_loader, dev, test_losses):
    network.eval()
    test_loss = 0
    correct = 0
    total = 0
    ...
    while ModelAvailable == False:
      ...
      for modelFile in param['Models']:
        ...
        if availableRecord >= param['TotalTrainNo'] * float(param['c']):
          ModelAvailable = True
      #print('Current Available Model Files are ', param['Models'])
  ...
  def SAFA_MQTT_Process(self, param):
