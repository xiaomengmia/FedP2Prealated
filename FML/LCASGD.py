...
def SSGD(param, config, seq, ComputingObject):
  ...
  network.weight_init()
  #  if ComputingObject.config['Protocol.Default'].find('MQTT') >=0:
  #    if param['ParameterServer'].find(ComputingObject.GetLocalIP()) >= 0:
  #      network = MergeModel(network, param, TotalBatchSize, dev, seq)
  #      GlobalModelFile = param['ProjectName'] + '-Global-' + str(param['CurrentEpoch']) + '.' + hostname + '.pth'
  #      SaveModelFile(network, GlobalModelFile, param)
  #      PushGlobalInfo(GlobalModelFile, param, ComputingObject)
  #      if GlobalModelFile not in param['GlobalModel']:
